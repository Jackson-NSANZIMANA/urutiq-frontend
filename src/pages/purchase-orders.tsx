...
export default function PurchaseOrdersPage() {
  ...
  const handleDownloadPDF = async (purchaseOrderId: string, poNumber: string) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:4000/api/purchase-orders/${purchaseOrderId}/pdf`,
        {
          method: 'GET',
          headers: {
  ...
  const handleSendToVendor = async (purchaseOrderId: string, vendorName: string) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:4000/api/purchase-orders/${purchaseOrderId}/send-to-vendor`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
  ...
  const handleMarkAsDelivered = async (order: PurchaseOrder) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:4000/api/purchase-orders/${order.id}/deliver`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
