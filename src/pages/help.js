...
export default function HelpPage() {
    ...
    const loadTutorialVideos = async () => {
        try {
            const response = await fetch('http://localhost:4000/api/tutorial-videos', {
                method: 'GET',
                headers: {
    ...
    const testUpload = async (file) => {
        try {
            ...
            const response = await fetch('http://localhost:4000/test-upload', {
                method: 'POST',
                body: formData
            });
            const result = await response.json();
    ...
    const handleUploadTutorialVideo = async (file) => {
        ...
        try {
            ...
            const response = await fetch('http://localhost:4000/api/tutorial-videos/upload', {
                method: 'POST',
                body: formData,
                headers: {
    ...
    const getVideoStreamUrl = (videoId) => {
        ...
        const companyId = localStorage.getItem('company_id') || 'cmg0qxjh9003nao3ftbaz1oc1';
        const token = localStorage.getItem('auth_token') || '';
        return `http://localhost:4000/api/tutorial-videos/stream/${videoId}?tenantId=${tenantId}&companyId=${companyId}&token=${token}`;
    };
    const addVideoTag = () => {
