...
export default function PurchaseOrdersPage() {
    ...
    const handleDownloadPDF = async (purchaseOrderId, poNumber) => {
        try {
            ...
            const response = await fetch(`http://localhost:4000/api/purchase-orders/${purchaseOrderId}/pdf`, {
                method: 'GET',
                headers: {
    ...
    const handleSendToVendor = async (purchaseOrderId, vendorName) => {
        try {
            ...
            const response = await fetch(`http://localhost:4000/api/purchase-orders/${purchaseOrderId}/send-to-vendor`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'x-tenant-id': tenantId,
    ...
    const handleMarkAsDelivered = async (order) => {
        try {
            ...
            const response = await fetch(`http://localhost:4000/api/purchase-orders/${order.id}/deliver`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'x-tenant-id': tenantId,
