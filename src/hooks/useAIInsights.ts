...
export function useAIInsights() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/ai-insights/dashboard?companyId=${companyId}`,
        {
          headers: {
  ...
  const generateInsights = async (insightType: string) => {
    ...
    const response = await fetch(
      `${import.meta.env.VITE_API_URL || 'http://localhost:4000'}/api/ai/insights/generate`,
      {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
